...
- **Daily digest reaches launch staff** — migration `0028`
  introduced the `staff_profile` table and migrated
  `apps/api/src/jp_adopt_api/domain/digest.py:_load_staff_recipients`
  off the contacts-join workaround. Joel + Amy are seeded with
  `digest_opt_in=true`, `status='active'`. Production is at head
  `0028`. The next digest tick after a match lands in the window
  delivers to both addresses.
- *Follow-up:* the `staff_seed` Contact rows added by 0027 are now
  orphaned. Benign (the digest no longer reads them, `party_kind='staff'`
  keeps them out of adopter/facilitator lists). A future migration
  can drop them.
...
3. **DNS rebind**: SWA → ACA web. Operator runs [`dns-rebind.md`](./runbooks/dns-rebind.md). Closes [#82](https://github.com/joshua-project/jp-adopt-core/issues/82).
4. **API external:false**: operator runs [`api-external-false.md`](./runbooks/api-external-false.md). Closes [#90](https://github.com/joshua-project/jp-adopt-core/issues/90). Depends on step 3.
5. **Backup-restore drill**: operator runs [`postgres-backup-restore.md`](./runbooks/postgres-backup-restore.md). **Non-negotiable** before any data cutover.
6. **Monitoring + alerting**: execute [plan #131](./plans/2026-06-10-001-monitoring-alerting-plan.md). IaC PR in `jp-infrastructure`, fire-the-alert drill from operator machine. Closes monitoring half of #91 Phase 2.
7. **Performance load test**: execute [plan #132](./plans/2026-06-10-002-performance-load-test-plan.md). Seeds synthetic data on staging, runs k6 scenarios, decides per-cliff fix-or-accept. Closes load-test half of #91 Phase 2.
...
8. **Staging dry-run**: per [DT import execution plan #122](./plans/2026-06-09-002-dt-contact-import-execution-plan.md) U2. Open MySQL firewall (operator IP), run `dt-etl --mode dry_run` against staging MySQL → local/staging Postgres. Triage any `unmapped_status:*` conflicts.
9. **Production snapshot dry-run**: same plan U3. Operator-led from your machine. Open prod MySQL firewall to operator IP; run dry-run against a fresh prod snapshot.
10. **Cutover window (Saturday)**: execute [`dt-cutover.md`](./runbooks/dt-cutover.md) step by step — 14:00 write freeze → 14:15 snapshot → 14:30 delta ETL → 15:00 verification → 16:00 flag flip → 17:00 announce. Plan #122 U4 references this runbook for the operator-side detail.
11. **Post-cutover verification**: plan #122 U5. Amy opens a real DT contact end-to-end in core.
...
12. **SWA decommission**: 14-day soak after Gate B step 3+4 → infra PR [jp-infrastructure#203](https://github.com/joshua-project/jp-infrastructure/issues/203) → adopt-core cleanup PR per [`dns-rebind.md` "Post-soak SWA decommission"](./runbooks/dns-rebind.md#post-soak-swa-decommission).
13. **DT final delta sync**: catch anything that landed in DT after Gate C's cutover. Same ETL, watermark-resumed.
14. **DT shutdown**: WordPress backup → DNS removal → hosting decommission → `dt-adoption-fields` plugin source archived.
15. **Communication plan**: partners + staff get the "DT retired" notice; quick-start link to [`quick-start.md`](./runbooks/quick-start.md).
